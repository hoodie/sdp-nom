...
named!{
    ...
    do_parse!(
        n: map_res!(take_while1!(is_not_space), |i: CompleteStr| u64::from_str_radix(&i, 10)) >>
        (n)
    )
}

named!{
    pub(crate) read_string<CompleteStr, CompleteStr>,
    do_parse!(
...
named!{
    pub(crate) read_as_strings<CompleteStr, Vec<CompleteStr>>,
